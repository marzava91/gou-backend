...
async function importBrands(tenantId: string) {
  ...
  if (rows.length === 0) {
    console.log(
      'ℹ️ marcas dummy.csv no encontrado o vacío; se omite import de marcas.',
    );
    return [];
  }
...
async function ensureCategoriesFromProducts(
  ...
  for (const p of products) {
    ...
    if (p.categoryParent && p.categoryChild)
      childPairs.add(`${p.categoryParent}|||${p.categoryChild}`);
  }

  console.log(
    `🗂️ Ensuring categories. Parents: ${parentNames.size}, child pairs: ${childPairs.size}`,
  );

  const parents = Array.from(parentNames);

  await withTiming(
    `Upserting parent categories (bulk=${parents.length})`,
    async () => {
  ...
  await withTiming(
    `Upserting child categories (bulk=${childData.length})`,
    async () => {
      await prisma.category.createMany({
        data: childData,
        skipDuplicates: true,
...
async function importProductsAsItems(params: {
  ...
  if (raw.length === 0)
    throw new Error('productos digitales.csv no encontrado o vacío.');

  const products: ProductRow[] = raw
    .map((r) => normalizeHeaderMap(r))
    .filter(
      (p) => p.ref && p.descripcion && p.categoryParent && p.categoryChild,
    );
    ...
    try {
      await prisma.$transaction(async (tx) => {
        for (let i = 0; i < batch.length; i++) {
          ...
          if ((i + 1) % 25 === 0) {
            console.log(
              `    • batch item ${i + 1}/${batch.length} (global ${processed + i + 1}/${products.length})`,
            );
          }
      ...
      console.error(
        `    ❌ Batch ${bi + 1} failed after ${fmtMs(now() - t0)} (processed=${processed})`,
      );
      console.error(e);

      // fallback: item por item para aislar fila
      console.log(
        '    🔎 Falling back to per-item processing to isolate the failing row...',
      );
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        try {
          ...
          if (processed % 25 === 0) {
            console.log(
              `    • fallback progress: ${processed}/${products.length}`,
            );
          }
        } catch (err) {
...
async function seedDummy(
  ...
  console.log(`📄 Files:`, {
    ...
    categories: fs.existsSync(FILE_CATEGORIES)
      ? FILE_CATEGORIES
      : 'OPTIONAL/NOT_USED',
  });
...
async function main() {
  ...
  console.log(
    `📌 Tip: para seed usa direct connection: $env:DATABASE_URL=$env:DIRECT_URL`,
  );
