...
describe('GrantCommandService', () => {
  ...
  it('creates ALLOW capability grant on active membership', async () => {
    ...
    grantsRepository.createGrant.mockResolvedValue({
      id: 'grant_1',
      membershipId: 'membership_1',
    ...
    expect(grantsRepository.createGrant).toHaveBeenCalledWith(
    ...
    expect(grantSupportService.publishEvent).toHaveBeenCalledWith(
      expect.objectContaining({ eventName: GrantDomainEvents.GRANT_CREATED }),
    );
  ...
  it('rejects duplicate active grant', async () => {
    ...
    grantsRepository.findDuplicateActiveGrant.mockResolvedValue({ id: 'grant_existing' });
  ...
  it('revokes active grant', async () => {
    ...
    grantsRepository.revokeGrant.mockResolvedValue(1);
...
import { GRANT_MEMBERSHIP_READER_PORT, GrantMembershipReaderPort } from '../ports/grant-membership-reader.port';
...
export class GrantCommandService {
  ...
  async createGrant(actorId: string | null, dto: CreateGrantDto) {
    ...
    const reason = this.grantSupportService.normalizeReason(dto.reason);

    const created = await this.grantsRepository.createGrant({
      membershipId: dto.membershipId,
      effect: dto.effect,
      targetType: dto.targetType,
      capabilityKey: normalizedTarget.capabilityKey,
  ...
  async revokeGrant(actorId: string | null, grantId: string, dto: RevokeGrantDto) {
    ...
    await this.grantsRepository.revokeGrant({
      id: grant.id,
...
import {
  GRANT_AUDIT_PORT,
  GrantAuditPort,
} from '../../ports/grant-audit.port';
import {
  GRANT_EVENTS_PORT,
  GrantEventsPort,
} from '../../ports/grant-events.port';
...
import { IsEnum, IsOptional, IsString, IsDateString, MaxLength, ValidateIf } from 'class-validator';
...
```ts
import { Injectable } from '@nestjs/common';
import { GrantEffect, GrantStatus, GrantTargetType, Prisma, PrismaClient } from '@prisma/client';
...
export class GrantsRepository {
  ...
  async findByIdOrThrow(id: string) {
    return this.prisma.grant.findUniqueOrThrow({ where: { id } });
  }
