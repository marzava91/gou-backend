...
export class StorePriceListsService {
  ...
  async get(tenantId: string, storeId: string, channel: SalesChannel, priceListId: string) {
    const row = await this.repo.findOne({ tenantId, storeId, channel, priceListId });
    ...
    try {
      ...
      if (isDefault) {
        ...
        const row = await prisma.$transaction(async (tx) => {
          await this.repo.clearDefaultForStoreChannel(tx, { tenantId, storeId, channel });
      ...
      const row = await this.repo.create(tenantId, { storeId, priceListId, channel, isDefault });
      return { data: toResponse(row) };
    } catch (e: any) {
      // PK compuesta storeId+priceListId+channel (y además tenantId en where)
      if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
        throw new ConflictException('StorePriceList already exists for this store/channel/priceList');
      }
      throw e;
    }
  }

  async update(tenantId: string, storeId: string, channel: SalesChannel, priceListId: string, dto: UpdateStorePriceListDto) {
    const exists = await this.repo.findOne({ tenantId, storeId, channel, priceListId });
    ...
    if (dto.isDefault === true) {
      const updated = await prisma.$transaction(async (tx) => {
        ...
        const res = await this.repo.setDefault(tx, { tenantId, storeId, channel, priceListId });
        if (res.count === 0) throw new Error('StorePriceList not found');

        return tx.storePriceList.findFirst({ where: { tenantId, storeId, channel, priceListId } });
      });
    ...
    const res = await this.repo.updateIsDefault(tenantId, { storeId, channel, priceListId, isDefault: false });
    if (res.count === 0) throw new NotFoundException('StorePriceList not found');

    const row = await this.repo.findOne({ tenantId, storeId, channel, priceListId });
    return { data: toResponse(row!) };
  }

  async remove(tenantId: string, storeId: string, channel: SalesChannel, priceListId: string) {
    ...
    const res = await this.repo.delete(tenantId, { storeId, channel, priceListId });
    if (res.count === 0) throw new NotFoundException('StorePriceList not found');
